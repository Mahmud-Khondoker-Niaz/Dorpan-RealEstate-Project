...
class User_Autologin extends CI_Model {
    ...
    private $users_table_name = 'siteuser';
    ...
    function get($user_id, $key) {
        $this->db->select($this->users_table_name . '.SiteuserID');
        $this->db->select($this->users_table_name . '.Username');

        $this->db->select($this->users_table_name . '.UserLevel');
        $this->db->select($this->users_table_name . '.Email');

        $this->db->select($this->users_table_name . '.Name');
        $this->db->select($this->users_table_name . '.Image');

        $this->db->select($this->users_table_name . '.BannerImage');
        $this->db->select($this->users_table_name . '.Country');
		$this->db->select($this->users_table_name . '.City');
        
        $this->db->from($this->users_table_name);
        $this->db->join($this->table_name, $this->table_name . '.user_id = ' . $this->users_table_name . '.SiteuserID');
