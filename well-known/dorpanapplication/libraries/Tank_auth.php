...
class Tank_auth {
            ...
            if (!is_null($user = $this->ci->users->$get_user_func($login))) { // login ok
                ...
                if ($hasher->CheckPassword($password, $user->Password)) {  // password ok
                    if ($user->banned == 1) {         // fail - banned
                        $this->error = array('banned' => $user->ban_reason . ' not confirm email yet.');
                    } elseif ($user->activated == 0 || $user->UserLevel == 'Pending') {         // fail - waiting
                        $this->error = array('not_activated' => 'This acount is in wait of approval.');
                    } elseif ($user->UserLevel == 'Blocked') {         // fail - banned
                        $this->error = array('banned' => 'This acount is blocked.');
                    } else {
					   
					   $date_raw =date('Y-m-d H:i:s');
					   
					   $timein =(date('Y-m-d H:i:s', strtotime('- 30 minutes', strtotime($date_raw))));
					
					    $this->ci->SiteModel->delete('usersitelogin', array('LoginTime <'=> $timein));

                        $minid = $user->SiteuserID . time() . rand(1, 999);

                        $this->ci->SiteModel->insert('usersitelogin', array('UsersiteloginID' => $minid, 'LoginTime' => date("Y-m-d H:i:s"), 'IpAddress' => $user->last_ip, 'IsUser' => 'Yes', 'SiteuserID' => $user->SiteuserID, 'Username' => $user->Username, 'Image' => $user->Image, 'Name' => $user->Name, 'UserAgent' => $this->ci->agent->browser(), 'IsMobile' => $this->ci->agent->is_mobile() ? 'IsUser' : 'No'));
						
						
						$usersiteloginInfo['UsersiteloginID']=$minid;
						$usersiteloginInfo['IsUser']='Yes';
						$usersiteloginInfo['SiteuserID']=$user->SiteuserID;
						$usersiteloginInfo['Username']=$user->Username;
						$usersiteloginInfo['Name']=$user->Name;
						$usersiteloginInfo['Image']=$user->Image;
						
						//$this->ci->SiteModel->update('usersitelogin',array('IpAddress'=>$user->last_ip,'SiteuserID'=>$user->SiteuserID),$usersiteloginInfo);
						
						

                        //$this->ci->SiteModel->insert('',);


                        $this->ci->session->set_userdata(array(
                            'user_id' => $user->SiteuserID,
                            'username' => $user->Username,
                            'user_level' => $user->UserLevel,
                            'user_email' => $user->Email,
                            'user_name' => $user->Name,
                            'user_thumb' => $user->Image,
                            'user_banner' => $user->BannerImage,
                        ...
                        if ($user->activated == 0) {       // fail - not activated
                            ...
                            if ($remember) {
                                $this->create_autologin($user->SiteuserID);
                            }

                            $this->clear_login_attempts($login);

                            $this->ci->users->update_login_info(
                                    $user->SiteuserID, $this->ci->config->item('login_record_ip', 'tank_auth'), $this->ci->config->item('login_record_time', 'tank_auth'));
                            return TRUE;
                        }
                    }
     ...
     */
    function logout() {
        $this->ci->SiteModel->delete('usersitelogin', array('SiteuserID' => $this->get_user_session('user_id')));
   ...
   function get_user_email()
	{
	 ...
	 function get_image()
	{
		return $this->ci->session->userdata('user_thumb');
	}
     ...
     * @return	string
     */
    function get_user_session($session_var) {
        return $this->ci->session->userdata($session_var);
    }
     ...
     * @return	array
     */
    function create_user($name,$name2, $username, $email, $password, $email_activation) {
        if ((strlen($username) > 0) AND ! $this->ci->users->is_username_available($username)) {
            $this->error = array('Username' => 'auth_username_in_use');
        } elseif (!$this->ci->users->is_email_available($email)) {
            $this->error = array('Email' => 'auth_email_in_use');
        } elseif (!$this->ci->users->is_name_available($name)) {
            $this->error = array('Name' => 'auth_name_in_use');
        } else {
    ...
    /**
     * Check if name available for registering.
     * Can be called for instant form validation.
     *
     * @param	string
     * @return	bool
    ...
    function is_name_available($name) {
        return ((strlen($name) > 0) AND $this->ci->users->is_name_available($name));
    }
    ...
    function forgot_password($login) {
        if (strlen($login) > 0) {
            if (!is_null($user = $this->ci->users->get_user_by_login($login))) {
                ...
                $this->ci->users->set_password_key($user->SiteuserID, $data['new_pass_key']);
                return $data;
            } else {
        ...
        if ((strlen($user_id) > 0) AND ( strlen($new_pass_key) > 0) AND ( strlen($new_password) > 0)) {
            ...
            if (!is_null($user = $this->ci->users->get_user_by_id($user_id, TRUE))) {
                ...
                if ($this->ci->users->reset_password(
                    ...
                    $this->ci->user_autologin->clear($user->SiteuserID);
