...
class Tank_auth
{
   ...
   function get_user_email()
	...
	{
		return $this->ci->session->userdata('user_branchid');
	}
         function get_branchname()
	{
		return $this->ci->session->userdata('user_branchname');
	}
	...
	/**
	 * Edit user on the site and return some data about it:
	 * user_id, username, password, email,activated,banned.
	 *
	 * @param	string
	 * @param	string
	 * @param	string
	...
	function edit_user($username, $email,   $banned, $id, $activated = NULL)    //added by lokman
	{
		if ((strlen($username) > 0) AND !$this->ci->users->is_username_available_edit($username, $id)) {
			$this->error = array('username' => 'auth_username_in_use');

		} elseif (!$this->ci->users->is_email_available_edit($email,$id)) {
			$this->error = array('email' => 'auth_email_in_use');
	...
	if ( $this->ci->users->update_user($data, $id)) {
