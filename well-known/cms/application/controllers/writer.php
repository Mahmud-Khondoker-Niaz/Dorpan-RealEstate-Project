...
class writer extends basecontroller{
   ...
   function view($id)
	{   
	 $data = $this->SiteModel->getfirstrow('writer','*', array('WriterID'=>$id), 'WriterID ASC', 1,0);
	...
	$data['info']	= $this->crdt;
			   ...
			   $data['menu']	= 4;
                            ...
                            $data['submenu']	= 'writer_list';
                           
                            $data['codemanage']	= "writer_list";
...
function writerentry($id=null)
	{
		...
		if (!$this->tank_auth->is_logged_in() ) {									// logged in
				...
				if(!is_null($id))  //while edit
				 {
			       ...
			       $data = $this->SiteModel->getfirstrow('writer','*', array('WriterID'=>$id), 'WriterID ASC', 1,0);
                             ...
                             $data['categorylist']  = array(""=>"Select","1"=>"Chairman","2"=>"Managing Director","3"=>"Other");
			 ...
			 else
			 	{
						...
						'Priority'=>'',
							'ContactNumber'=>'',
                                                    'Category'=>'',
						'categorylist' => array(""=>"Select","1"=>"Chairman","2"=>"Managing Director","3"=>"Other"),  
						"selected_item" => "",
						"IsActive" => 1
			...
			$this->form_validation->set_rules('WriterName', 'Name', 'trim|required|xss_clean');
                        $this->form_validation->set_rules('Email', 'Email', 'trim|valid_email|xss_clean');
		 			
			 
				if ($this->form_validation->run()) {	
	 
			 unset($data['WriterID'], $data['categorylist'], $data['selected_item'], $data['Submit']); 
				...
				if($id == null || $id == "")
						{
							...
							$id=$this->SiteModel->insert('writer',$data);
							$msg = 'Saved Successfully';
						}
						else
						{
							$this->SiteModel->update('writer',array('WriterID'=>$id), $data);
   ...
   base_url().'assets/plugins/jquery-slimscroll/jquery.slimscroll.min.js',
    ...
    base_url().'assets/plugins/uniform/jquery.uniform.min.js',
	  ...
	  base_url().'assets/scripts/custom/form-validation.js'
			...
			$this->load->view('head', $data);
			$this->load->view('writer/recordentrywriter');
			$this->load->view('foot');
...
function emailcheck($email, $id)
 {
  ...
  $rowcount =  $this->SiteModel->getcolumn('siteuser','COUNT(WriterID)', array('WriterID !=' =>$id, 'Email'=>$email),'WriterID DESC', 1, 0);
 ...
 if ($rowcount > 0)
  ...
  return true;	
	...
	function getlist()
	{
         ...
         */
        $aColumns = array('WriterID', 'WriterName', 'Image' , 'Email', 'IsActive', 'Category' );
	...
	///////////////////deleting records
		...
		if (isset($_REQUEST["sAction"]) && $_REQUEST["sAction"] == "group_action" && $_REQUEST["sGroupActionName"] == "Cancel") {
			...
			foreach($_REQUEST["id"] as $record_id){
				  $this->db->delete($sTable, array('WriterID' => $record_id));
			$flag=1;
				}
		...
		if (isset($_REQUEST["sAction"]) && $_REQUEST["sAction"] == "group_action" && $_REQUEST["sGroupActionName"] == "IsActive") {
			...
			foreach($_REQUEST["id"] as $record_id){
				  $this->db->update($sTable, array("IsActive"=>1), array('WriterID' => $record_id));
			$flag=10;
				}
	...
	///////////////////InActivating records
		...
		if (isset($_REQUEST["sAction"]) && $_REQUEST["sAction"] == "group_action" && $_REQUEST["sGroupActionName"] == "InActive") {
			...
			foreach($_REQUEST["id"] as $record_id){
				  $this->db->update($sTable, array("IsActive"=>0), array('WriterID' => $record_id));
			$flag=20;
				}
        ...
        { 
            $row = array();
		 	...
		 	$row[] = '<input type="checkbox" name="id[]" value="'.$aRow['WriterID'].'">'; 
            ...
            foreach($aColumns as $col)
            {
			  ...
			  $row[] = '<a href="'.site_url("writer/writerentry/".$aRow['WriterID']).'" class="btn btn-xs default"><i class="fa fa-edit"></i> Edit</a> <a data-target="#ajax" data-toggle="modal" href="'.site_url("writer/view/".$aRow['WriterID']).'" class="btn btn-xs default"><i class="fa fa-search"></i> View</a>';
