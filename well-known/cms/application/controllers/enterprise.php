...
class enterprise extends basecontroller{
   ...
   function view($id)
	{   
	 $data = $this->SiteModel->getfirstrow('enterprise','*', array('EnterpriseID'=>$id), 'EnterpriseID ASC', 1,0);
	$this->load->view('enterprise/view',$data);
	}
	...
	$data['info']	= $this->crdt;
			   ...
			   $data['menu']	= 102;
							 $data['title']	= "Enterprise List";
                            $data['submenu']	= 'enterprise_list1';
                           
                            $data['codemanage']	= "enterprise_list";
    ...
    base_url().'assets/plugins/uniform/jquery.uniform.min.js',
	  ...
	  base_url().'assets/scripts/core/datatable.js',
	   base_url().'assets/scripts/custom/ajaxdatatable/table-ajax.js' 
			...
			$this->load->view('enterprise/index');
			$this->load->view('foot');				
...
function enterpriseentry($id=null)
	{
		...
		if (!$this->tank_auth->is_logged_in() ) {									// logged in
			...
			$this->form_validation->set_rules('EnterpriseName', 'Name', 'trim|required|xss_clean');
                        $this->form_validation->set_rules('Email', 'Email', 'trim|valid_email|xss_clean');
		 			
			 
				if ($this->form_validation->run()) {	
	 
			 unset($data['EnterpriseID'], $data['Submit']); 
				...
				if($id == null || $id == "")
						{
							...
							$id=$this->SiteModel->insert('enterprise',$data);
							$msg = 'Saved Successfully';
						}
						else
						{
							$this->SiteModel->update('enterprise',array('EnterpriseID'=>$id), $data);
   ...
   base_url().'assets/plugins/jquery-slimscroll/jquery.slimscroll.min.js',
    ...
    base_url().'assets/plugins/uniform/jquery.uniform.min.js',
	  ...
	  base_url().'assets/scripts/custom/form-validation.js'
			...
			$this->load->view('head', $data);
			$this->load->view('enterprise/recordentryenterprise');
			$this->load->view('foot');
	...
	function getlist()
	{
         ...
         */
        $aColumns = array('EnterpriseID', 'EnterpriseName', 'Image' ,  'IsActive');
	...
	///////////////////deleting records
		...
		if (isset($_REQUEST["sAction"]) && $_REQUEST["sAction"] == "group_action" && $_REQUEST["sGroupActionName"] == "Cancel") {
			...
			foreach($_REQUEST["id"] as $record_id){
				  $this->db->delete($sTable, array('EnterpriseID' => $record_id));
			$flag=1;
				}
		...
		if (isset($_REQUEST["sAction"]) && $_REQUEST["sAction"] == "group_action" && $_REQUEST["sGroupActionName"] == "IsActive") {
			...
			foreach($_REQUEST["id"] as $record_id){
				  $this->db->update($sTable, array("IsActive"=>1), array('EnterpriseID' => $record_id));
			$flag=10;
				}
	...
	///////////////////InActivating records
		...
		if (isset($_REQUEST["sAction"]) && $_REQUEST["sAction"] == "group_action" && $_REQUEST["sGroupActionName"] == "InActive") {
			...
			foreach($_REQUEST["id"] as $record_id){
				  $this->db->update($sTable, array("IsActive"=>0), array('EnterpriseID' => $record_id));
			$flag=20;
				}
        ...
        { 
            $row = array();
		 	...
		 	$row[] = '<input type="checkbox" name="id[]" value="'.$aRow['EnterpriseID'].'">'; 
            ...
            foreach($aColumns as $col)
            {
			  ...
			  $row[] = '<a href="'.site_url("enterprise/enterpriseentry/".$aRow['EnterpriseID']).'" class="btn btn-xs default"><i class="fa fa-edit"></i> Edit</a> <a data-target="#ajax" data-toggle="modal" href="'.site_url("enterprise/view/".$aRow['EnterpriseID']).'" class="btn btn-xs default"><i class="fa fa-search"></i> View</a>';
