...
class menu extends basecontroller{
	...
	$this->load->view('menusection/view');
	     ...
	     function poweredit($menusectionid)
	{
		 
 
                 
                 $this->session->set_userdata(array("menusectionid"=>$menusectionid));
                
                 $data["nestedsubmenu"] = $menusectionid;
		 if ($this->tank_auth->is_logged_in() && ( $this->tank_auth->get_userlevel()==9 )) {
		   ...
		   if ($message = $this->session->flashdata('message')) {
			 		     $data['message']	= $message;
					   		 ...
					   		 $data['menu']	= 2;
							 $data['title']	= "Menu Item Order";
                             $data['submenu']	= 'menusection_list';
                             $data['codemanage']	= "menusection_list";
			   
                             
               		
                $data['powermenulist'] = $this->SiteModel->getlist('menu','MenuID as sect_id, ParentID as  sect_parent, Caption as sect_name', array('MenuGroup' =>$menusectionid),'Priority ASC', 500, 0);               
   ...
   base_url().'assets/plugins/jquery-slimscroll/jquery.slimscroll.min.js',
        ...
        function index()
	{
                 ...
                 $menusectionid = $this->uri->segment(2);
...
function menuentry($id=null)
	{
		...
		if (!$this->tank_auth->is_logged_in() && $this->tank_auth->get_userlevel() !=9) {									// logged in
			...
			redirect('/auth/send_again/');
			 ...
			 $menusectionid =   $this->session->userdata("menusectionid");
			...
			if(!isset($_POST) || empty($_POST)) {
				...
				if(!is_null($id))  //while edit
				 {
			       ...
			       $data = $this->SiteModel->getfirstrow('menu','*', array('MenuID'=>$id), 'Caption ASC', 1,0);
			       $data['menulist']  = $this->dropdown($this->SiteModel->getlist('menu','MenuID as ID, Caption as Name', array('MenuGroup' => $menusectionid),'Caption ASC', 1000, 0));
				...
				$data["selected_item"] = $data["ParentID"]; 
			 ...
			 else
			 	{
						...
						'MenuLink'=>'',	
						"ParentID" => "",
						'menulist' => $this->dropdown($this->SiteModel->getlist('menu','MenuID as ID, Caption as Name', array('MenuGroup' => $menusectionid),'Caption ASC', 1000, 0)),  
						"selected_item" => "",
						"Priority" => "" ,
						"IsActive" => 1 ,
						"MenuGroup" => $menusectionid,
			...
			$this->form_validation->set_rules('Priority', 'Priority', 'trim|required|xss_clean');				 
			 ...
			 unset($data['MenuID'], $data['menulist'], $data['selected_item'], $data['Submit']); 
				...
				if($id == null || $id == "")
						{
							...
							$id=$this->SiteModel->insert('menu',$data);
							$msg = 'Saved Successfully';
						}
						else
						{
							$this->SiteModel->update('menu',array('MenuID'=>$id), $data);
							$msg = 'Updated Successfully'; 
						}
			
		 	//$this->saveImages($id);
			 $this->_show_msg($msg, 'menu/'.$menusectionid);
			}
   ...
   base_url().'assets/plugins/jquery-slimscroll/jquery.slimscroll.min.js',
    ...
    base_url().'assets/plugins/uniform/jquery.uniform.min.js',
	  ...
	  base_url().'assets/scripts/custom/form-validation.js'
			...
			$this->load->view('head', $data);
			$this->load->view('menu/recordentrymenu');
			$this->load->view('foot');
        ...
        function getlist()
	{
         ...
         */
        $aColumns = array('MenuID', 'Caption','MenuLink', 'Priority','IsActive','LinkType');
	...
	///////////////////deleting records
		...
		if (isset($_REQUEST["sAction"]) && $_REQUEST["sAction"] == "group_action" && $_REQUEST["sGroupActionName"] == "Cancel") {
			...
			foreach($_REQUEST["id"] as $record_id){
				  $this->db->delete($sTable, array('MenuID' => $record_id));
			$flag=1;
				}
		...
		if (isset($_REQUEST["sAction"]) && $_REQUEST["sAction"] == "group_action" && $_REQUEST["sGroupActionName"] == "IsActive") {
			...
			foreach($_REQUEST["id"] as $record_id){
				  $this->db->update($sTable, array("IsActive"=>1), array('MenuID' => $record_id));
			$flag=10;
				}
	...
	///////////////////InActivating records
		...
		if (isset($_REQUEST["sAction"]) && $_REQUEST["sAction"] == "group_action" && $_REQUEST["sGroupActionName"] == "InActive") {
			...
			foreach($_REQUEST["id"] as $record_id){
				  $this->db->update($sTable, array("IsActive"=>0), array('MenuID' => $record_id));
			$flag=20;
				}
         ...
         */
                  ...
                  $this->db->where("MenuGroup", $menusectionid);   
...
//      '<span class="label label-sm label-'.(key($status)).'">'.(current($status)).'</span>',
 //     '<a href="javascript:;" class="btn btn-xs default"><i class="fa fa-search"></i> View</a>',
			...
			$row[] = '<input type="checkbox" name="id[]" value="'.$aRow['MenuID'].'">'; 
            ...
            foreach($aColumns as $col)
            {
			  ...
			  $row[] = '<a href="'.site_url("menu/menuentry/".$aRow['MenuID']).'" class="btn btn-xs default"><i class="fa fa-edit"></i> Edit</a> <a data-target="#ajax" data-toggle="modal" href="'.site_url("menu/view/".$aRow['MenuID']).'" class="btn btn-xs default"><i class="fa fa-search"></i> View</a>';
	...
	function _checkcaption($caption, $id)
	{
	 ...
	 $this->SiteModel->update('menu',array('MenuID'=>$optionArray[$i-1]["id"]),array('ParentID'=> $optionArray[$i-1]['parentID'], 'Priority'=> $i) );
   			 }
