		...
		*/
		...
		/*else */
		 ...
		 if (!$this->config->item('allow_registration', 'tank_auth')) {	// registration is off
			...
			//echo $this->form_validation->set_value('BranchID');						// validation ok
				if (!is_null($data = $this->tank_auth->create_user(
						$use_username ? $this->form_validation->set_value('username') : '',
						$this->form_validation->set_value('email'),
						$this->form_validation->set_value('password'),
					...
					//$this->saveImages($id = $data["user_id"]);
					...
					if ($email_activation) {									// send "activate" email
						...
						//$this->_show_message($this->lang->line('auth_message_registration_completed_1'));
						 ...
						 $this->_show_msg($this->lang->line('auth_message_registration_completed_1'));  //lokman
						...
						//$this->_show_message($this->lang->line('auth_message_registration_completed_2').' '.anchor('/siteadmin/login/', 'Login'));
						 $this->_show_msg($this->lang->line('auth_message_registration_completed_2').' '.anchor('/siteadmin/login/', 'Login'));
					}
				} else {
					$errors = $this->tank_auth->get_error_message();
					foreach ($errors as $k => $v)	$data['errors'][$k] = $this->lang->line($v);
   ...
   base_url().'assets/plugins/jquery-slimscroll/jquery.slimscroll.min.js',
	...
	function activate()
	{
		...
		if ($this->tank_auth->activate_user($user_id, $new_email_key)) {		// success
			$this->tank_auth->logout();
			$this->session->sess_create();
			$this->_show_message($this->lang->line('auth_message_activation_completed').' '.anchor('/siteadmin/login/', 'Login'));
	...
	{
		if ( ( $this->tank_auth->get_userlevel() !=9 ) &&  $this->tank_auth->is_logged_in()) {									// logged in
			redirect('');
	...
	function reset_password()
	{
		...
		if ($this->form_validation->run()) {								// validation ok
			if (!is_null($data = $this->tank_auth->reset_password(
				...
				$this->_show_message($this->lang->line('auth_message_new_password_activated').' '.anchor('/siteadmin/login/', 'Login'));
	...
	function reset_email()
	{
		...
		if ($this->tank_auth->activate_new_email($user_id, $new_email_key)) {	// success
			$this->tank_auth->logout();
			$this->session->sess_create();
			$this->_show_message($this->lang->line('auth_message_new_email_activated').' '.anchor('/siteadmin/login/', 'Login'));
