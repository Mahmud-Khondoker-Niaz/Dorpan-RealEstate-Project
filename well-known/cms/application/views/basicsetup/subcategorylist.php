	...
	<?php } ?>*/
        ...
        $(document).ready(function () {
		    ...
		    var data = new Array();
					  ...
					  var icount=0;
					  $.get('<?php echo base_url();?>basicsetup/subcategorylist', function(data12) {													
			  
			
					 $.each(data12, function(key, val) {
			 
						 
			            data[icount] =  {'ID':val.SubcategoryID, 'Delete':val.SubcategoryID, 'SubcategoryName':val.SubcategoryName, 'Priority':val.Priority, 'Category':val.CategoryName,  'titleofcourtesy':'ggg', 'birthdate':'123', 'hiredate':'f76', 'address':'www',  'Active':(val.IsActive==1)?"Yes":"No", 'last_login':'2012-11-12','created':'ddd','notes':'Notes Here'} ;
		   ...
		   var dbutton = function (row, datafield, value) {
			   //alert(value);
                 ...
                 return '<div style="margin:6px 10px;  width:70px; height:23px;" class="jqx-rc-all jqx-button jqx-widget jqx-fill-state-normal"><a   onClick="return confirm(\'Are you sure?\')" href="<?php echo base_url();?>basicsetup/subcategorydelete/'+value+'"     style="height:23px; line-height:23px; color:#000;" >Delete</a></div>';
            }
            ...
            var source =
            {
				...
				deleterow: function (rowid, commit) {
									   ...
									   var flag = '';
										$("#indicator").css('display','block');
											$.ajax({
																		dataType: 'html',
																		type: 'post',
																		url: '<?php echo base_url();?>basicsetup/subcategorydeleterow',
																		data: datarow,
			      ...
			      updaterow: function (rowid, rowdata, commit) {
                    ...
                    var datarow = "update=1&SubcategoryName=" + rowdata.SubcategoryName + '&priority='+priority_item+'&activated='+active_item;
                    datarow = datarow + "&category_id=" + selectedUrunId + "&Item_ID=" + rowdata.ID;
				 ...
				 // $("#indicator").fadeIn(200).html('<img src="http://localhost/bdblog/siteimages/ajax-loader.gif" />');
				   $("#indicator").css('display','block');
				    $.ajax({
                        dataType: 'html',
						type: 'post',
                       	url: '<?php echo base_url();?>basicsetup/updatesubcategoryrow',
                        data: datarow,
            ...
            var initrowdetails = function (index, parentElement, gridElement, datarecord) {
                ...
                if (tabsdiv != null) {
                   ...
                   // title.text(datarecord.ID);
                    ...
                    var SubcategoryName = "<div style='margin: 10px;'><b>  Subcategory Name:</b> " + datarecord.SubcategoryName + "</div>";
                    ...
                    $(leftcolumn).append(SubcategoryName);
            ...
            var dataAdapter = new $.jqx.dataAdapter(source);
					...
					var twodimension = new Array();
					
					  var icount=0;
					  $.get('<?php echo base_url();?>basicsetup/category_ajax', function(data12) {													
			  
			 
					 $.each(data12, function(key, val) {
					var onedimension = new Array();
						//items[0] = val.BlogpostID;
			           // items[icount] = val.CategoryID;
						// itemsname[icount] = val.CategoryName;
						   onedimension['value'] = val.CategoryID;
						   onedimension['label'] = val.CategoryName;
						  
							twodimension.push(onedimension);
               ...
               //  height: 500,
				...
				renderstatusbar: function (statusbar) {
				   ...
				   container2.append(addButton);
                    ...
                    addButton.click(function (event) {
                         window.location = '<?php echo base_url();?>basicsetup/subcategoryentry';
				...
				// try to add modal yes no 
                    ...
                    refreshbutton.click(function (event) {
										  ...
										  var icount1=0;
										  $.get('<?php echo base_url();?>basicsetup/subcategorylist', function(data122) {													
										 ...
										 $.each(data122, function(key, val) {
											...
											data2[icount1] =  {'ID':val.SubcategoryID, 'Delete':val.SubcategoryID, 'SubcategoryName':val.SubcategoryName, 'Priority':val.Priority, 'Category':val.CategoryName,  'titleofcourtesy':'ggg', 'birthdate':'123', 'hiredate':'f76', 'address':'www',  'Active':(val.IsActive==1)?"Yes":"No", 'last_login':'2012-11-12','created':'ddd','notes':'Notes Here'} ;
                ...
                columns: [
                      ...
                      { text: 'Subcategory', columntype: 'textbox',  datafield: 'SubcategoryName', width: 100 },
					  { text: 'Category', columntype: 'textbox',  datafield: 'Category', width: 100, columntype: 'dropdownlist',
					   createeditor: function (row, cellvalue, editor) {
											  var urunId = $('#jqxgrid').jqxGrid('getcellvalue', row, "Category");
											 
											editor.jqxDropDownList({source: twodimension, displayMember: 'label', valueMember: 'value', autoDropDownHeight: true });
											
  $(document).on('select', editor, function (event) {
 
 selectedUrunId =  twodimension[editor.jqxDropDownList('getSelectedIndex')]['value'];
		...
		/*	 $("#deleterowbutton").jqxButton({ theme: theme });
            ...
            $("#dropdownlist").jqxDropDownList({ autoDropDownHeight: true, selectedIndex: 0, width: 200, height: 23, theme: theme,
                source: [
                    'Subcategory',  'Category',  'Priority',   'Active?'
                ]
            });
    ...
    <script type="text/javascript">
            var buttonclick = function (id) {
				...
				window.location = '<?php echo base_url();?>basicsetup/subcategoryentry/'+buttonID;
			    //alert(buttonID);
            }
			...
			var dbuttonclick = function (id) {
				...
				window.location = '<?php echo base_url();?>basicsetup/subcategorydelete/'+buttonID;
			    //alert(buttonID);
            }
